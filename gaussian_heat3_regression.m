...
theta_H_mu0 = theta_H(mu0);
plot(xis,F2X([x0,x0],theta_H_mu0(1:qH)),'--',"DisplayName","Taylor approx "+qH)
...
theta_H_mu1 = theta_H(mu1);
plot(xis,F2X([x0,x0],theta_H_mu1(1:qH)),'--',"DisplayName","Taylor approx "+qH)
...
for j = 1:nn
    ...
    hA2s_ = zeros(n_,n_is_,qH_);
    tA2s_ = zeros(n_,n_is_,qH_);
    ...
    if decoupled
        deco.Os = reshape(deco.O,[n_, n_is_(1), qH_]);
    ...
    if monolithic
        mono.Os = reshape(mono.O,[n_, n_is_(1), qH_]);
