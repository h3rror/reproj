...
inds = ceil(eps+d*((A*xis)'-Omega(1))/(Omega(2)-Omega(1)));
...
% nu = @(x,mu) [0;x;0].*mu(inds); % x argument for compatibility
% nu = @(x,mu) (A*[0;x;0]).*mu(inds); % x argument for compatibility
nu = @(x,mu) mu(inds); % x argument for compatibility
...
plot(A*xis,nu(x0,mu0), "DisplayName","\nu(x_0,\mu_0)")
...
B = @(x,mu) -1/dx*spdiags([-Iminus*nu([],mu) (Iminus+Iplus)*nu([],mu) -Iplus*nu([],mu)],[-1 0 1],N1,N1);

% F1_exact = @(x,mu) C\B*(nu([],mu).*x);
% F1_exact = @(x,mu) C\(B(x,mu)*x);

% F2_exact = @(x1,x2,mu) C\(B(x1,mu)*x2);
F2_exact = @(x1,x2,mu) C\(B([],mu)*(x1.*x2));
% F2_exact = @(x1,x2,mu) C\(B(x0,mu)*x2);
F2X_exact = @(X,mu) F2_exact(X(:,1),X(:,2),mu); % enable storing variables in one matrix
F2X = @(X,mu) F2X_exact(X,mu);
