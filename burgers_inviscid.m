...
for j = 1:nn
    ...
    if computeROMStateError
        ...
        tf = @(tx,u) tA2_*uniquepower(tx,2);
        hf = @(hx,u) hA2_*uniquepower(hx,2);
...
save("data/data_burgers_inviscid","O_errors","condsD");

%%

figure(11)
semilogy(ns,O_errors,'*-', 'LineWidth', 2,'DisplayName',"Burger's inviscid", "MarkerSize",10)
figure(12)
semilogy(ns,condsD,'*-', 'LineWidth', 2,'DisplayName',"Burger's inviscid", "MarkerSize",10)
