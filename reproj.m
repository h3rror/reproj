...
for n=nList
    ...
    %% Exact operator inference with data recycling
    [Q,R,P]=qr(XtrainProj(:,1:end-1),"econ"); % exclude last column since that one does not have a successor
    r = rank(XtrainProj);
    [p,~] = find(P(:,1:r));
    [Vr,~,~] = svd(Xtrain(:,p),'econ');
    ArOpInfRecy = ((Vr'*Xtrain(:, p))'\(Vr'*Xtrain(:, p+1))')';
    ArRecy = Vr' *A*Vr;
    norm(ArOpInfRecy-ArRecy)


% %% Store results for analysis
%     save(['results_n_', num2str(n), '.mat'], 'XtrainIntMOR', 'XtestIntMOR', 'XtrainOpInf', 'XtestOpInf', 'XtrainReProj', 'XtestOpInfReProj');
% 
